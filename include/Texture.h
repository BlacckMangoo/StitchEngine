...
public:
    ...
    Texture(const int width, const int height, const TextureDescription &desc,const unsigned char* data = nullptr) : width(width), height(height),
		...
		assert(mipLevels == 1 || desc.hasMipmap && mipLevels > 1 && "Invalid mip level calculation!");

        // note for future :  some things require empty textures like frame buffers so  separated allocation from filling
        if (data) {
...
private:
    ...
    void AllocateStorage(int width , int height) {
        ...
        glTextureStorage2D(
            id,
            mipLevels,
            textureDescription.internalformat,
            width,
            height
        );
	}
