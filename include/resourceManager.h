...
struct Material {
  Shader *shader{};
  std::unordered_map<std::string, UniformValue> uniforms; // "nameInShaderCode", value
  std::unordered_map<std::string, std::pair<Texture*, Sampler*>> textures; //  "nameInShaderCode",Texture
};


inline std::vector<float> GetInterleavedPosTex(const fastgltf::Asset &asset,
...
inline std::vector<GlPrimitive> LoadPrimitivesFromGltf(const fastgltf::Mesh &mesh,
...
struct ResourceManager {
  ...
  void LoadShader(const std::string &vertPath, const std::string &fragPath,
    ...
    if (!shaders.contains(key)) {
      shaders[key] = std::make_shared<Shader>(GetFileText(vertPath),
                                              GetFileText(fragPath));
    }
    ...
    if (!materials.contains(key)) {
      ...
      for (const auto &[uniformName, textureInfo] : textures) {
        ...
        material->textures[uniformName] = {
            texture, sampler}; // example : "albedoMap", {Texture*, Sampler*}
      }
